...
function PackageGenerationPanel({ projectId, assessmentId }: { projectId: string; assessmentId: string }) {
  ...
  return <section className="package-generation-panel" aria-labelledby="package-generation-title">
    ...
    {packages[0] && <IssueFinalDeliverablesPanel projectId={projectId} pkg={packages[0]} signed={reviews[packages[0].id]?.state === "Ready to issue"} />}
  </section>;
}

function IssueFinalDeliverablesPanel({ projectId, pkg, signed }: { projectId: string; pkg: GeneratedPackage; signed: boolean }) {
