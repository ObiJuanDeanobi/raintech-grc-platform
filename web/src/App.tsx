...
function PackageGenerationPanel({ projectId, assessmentId }: { projectId: string; assessmentId: string }) {
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      const [nextReadiness, nextPackages] = await Promise.all([
        ...
        request<GeneratedPackage[] | { packages?: GeneratedPackage[] }>(`/api/projects/${projectId}/assessments/${assessmentId}/packages`, { signal }),
      ]);
      if (sequence !== sequenceRef.current || signal?.aborted) return;
      setReadiness(nextReadiness);
      setPackages(Array.isArray(nextPackages) ? nextPackages : nextPackages.packages ?? []);
    } catch (caught) {
  ...
  async function generate() {
    ...
    try {
      await request(`/api/projects/${projectId}/assessments/${assessmentId}/packages/generate`, { method: "POST" });
  ...
  return <section className="package-generation-panel" aria-labelledby="package-generation-title">
    ...
    {packages.length > 0 && <div className="generated-package-list"><strong>Generated packages</strong>{packages.map((pkg) => <article key={pkg.id} className="generated-package"><div><strong>{pkg.state === "promoted" ? "Complete package" : `Package ${pkg.state}`}</strong><small>{new Date(pkg.created_at).toLocaleString()} · Source {pkg.source_snapshot_id ?? "snapshot recorded"}</small></div><div className="generated-components">{pkg.components.map((component) => <a key={component.id} className="text-button" href={component.download_url ?? `/api/projects/${projectId}/assessments/${assessmentId}/packages/${pkg.id}/components/${component.id}/download`}>{component.filename || component.kind}</a>)}</div>{pkg.source_sha256 && <small>Source SHA-256: {pkg.source_sha256}</small>}</article>)}</div>}
  </section>;
}
