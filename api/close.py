...
def _block(
    items: list[dict[str, Any]], code: str, detail: str, check: str, link: str | None = None
) -> None:
...
def _approved_profile(connection: Any, project: Any) -> Any:
    ...
    return connection.execute(
        """SELECT versions.* FROM profile_versions versions
...
def _scope_items(
    connection: Any, profile_id: str, declaration: dict[str, Any]
) -> list[dict[str, str]]:
    ...
    for target in declaration.get("scope_targets", []):
        ...
        if types:
            ...
            rows = connection.execute(
                f"""SELECT items.client_key,
                COALESCE(
                    MAX(CASE WHEN values_row.field_key = 'name' THEN values_row.value END),
                    items.client_key
                ) AS name
                FROM profile_items items
                LEFT JOIN profile_field_values values_row
                  ON values_row.profile_item_id = items.id
                 AND values_row.profile_version_id = items.profile_version_id
                WHERE items.profile_version_id = ? AND items.item_type IN ({placeholders})
                GROUP BY items.id, items.client_key, items.sort_order ORDER BY items.sort_order""",
                (profile_id, *types),
            ).fetchall()
            items.extend(
                {
        ...
        if sections:
            ...
            rows = connection.execute(
                f"""SELECT section, field_key, label, value
...
def fieldwork_ready(
    ...
    if framework is None:
        raise HTTPException(status_code=409, detail="Framework declaration not found")
    declarations = json.loads(framework["declarations_json"])
    close_decl = declarations.get("close_readiness", {}).get(TARGET)
    if not isinstance(close_decl, dict):
        raise HTTPException(
            status_code=409, detail="Close target is not declared by this framework"
        )
    validator_names = close_decl.get("validators", [])
    final_statuses = set(close_decl.get("final_statuses", []))
    if not final_statuses or not final_statuses <= set(declarations.get("status_set", [])):
        raise HTTPException(
            status_code=409, detail="Close target final statuses are not valid for this framework"
        )
    not_met_status = close_decl.get("not_met_status")
    na_status = close_decl.get("not_applicable_status")
    blockers: list[dict[str, Any]] = []
    for name in validator_names:
        if name not in _CHECKS:
            _block(
                blockers, "unknown_validator", f"Unsupported close validator: {name}", "declaration"
            )
    ...
    if latest is None or latest["next_state"] != completion.get("state"):
        _block(
            blockers,
    ...
    if latest is not None:
        for field in json.loads(latest["unresolved_required_fields_json"] or "[]"):
            _block(
                blockers,
                "profile_unknown",
                f"Resolve required field: {field}",
                "profile",
                f"/projects/{project_id}/profile",
            )
    if approved is not None:
        unknown = connection.execute(
            """SELECT field_key FROM profile_field_values
               WHERE profile_version_id = ?
                 AND lower(trim(value)) IN (
                     'unknown', 'to determine', 'unknown / to determine'
                 )""",
            (approved["id"],),
        ).fetchall()
        for row in unknown:
    ...
    if assessment is None:
        ...
        records = connection.execute(
            """SELECT record_id, carries_determination FROM framework_records
        ...
        for record_id in [row["record_id"] for row in records if row["carries_determination"]]:
            ...
            status = det["status"] if det else ""
            link = f"/projects/{project_id}/assessments/{assessment_id}/records/{record_id}"
            if status not in final_statuses:
            ...
            elif status == na_status and not (det["na_rationale"] or "").strip():
                _block(
                    blockers,
                    "na_rationale_missing",
                    f"N/A rationale required: {record_id}",
                    "determinations",
                    link,
                )
            elif status == not_met_status:
                ...
                if rec is None or not rec["finding_id"] or not rec["corrective_action_id"]:
                    _block(
                        blockers,
                        "not_met_unreconciled",
                        f"Reconcile Not Met: {record_id}",
                        "reconciliation",
        ...
        if not isinstance(sra, dict):
            _block(
                blockers,
                "sra_not_declared",
                "SRA is not declared by the framework",
                "sra",
                f"/projects/{project_id}/sra",
            )
        else:
            anchor = connection.execute(
                "SELECT 1 FROM framework_records WHERE framework_version_id = ? AND record_id = ?",
                (project["framework_version_id"], sra.get("anchor_record_id", "")),
            ).fetchone()
            if anchor is None:
                _block(
                    blockers,
                    "sra_anchor_missing",
                    "Declared SRA anchor does not resolve",
                    "sra",
                    f"/projects/{project_id}/sra",
                )
            if approved is not None:
                ...
                if not scope_items:
                    _block(
                        blockers,
                        "sra_incomplete",
                        "Approved Profile contains no declared SRA scope facts",
                        "sra",
                        f"/projects/{project_id}/sra",
                    )
                    ...
                    review = connection.execute(
                        """SELECT * FROM sra_scope_reviews
                           WHERE project_id = ? AND profile_version_id = ?
                             AND scope_type = ? AND target_key = ?""",
                        (project_id, approved["id"], item["scope_type"], item["target_key"]),
                    ).fetchone()
                    if review is None:
                        _block(
                            blockers,
                        ...
                        not review["included"] and not (review["exclusion_rationale"] or "").strip()
                    ):
                        _block(
                            blockers,
                            "sra_exclusion_rationale_missing",
                            f"Explain excluded {item['scope_type']} scope: {item['name']}",
                            "sra",
                            f"/projects/{project_id}/sra/scope",
                        )
                ...
                if not risks:
                    _block(
                        blockers,
                        "risk_incomplete",
                        "Record at least one complete SRA risk",
                        "sra",
                        f"/projects/{project_id}/risks",
                    )
                for risk in risks:
                    missing = [
                        field for field in required_fields if not str(risk[field] or "").strip()
                    ]
                    if missing:
                        _block(
                            blockers,
                            "risk_incomplete",
                            f"Complete risk {risk['title']}: {', '.join(missing)}",
                            "sra",
                ...
                _block(
                    blockers,
                    "sra_not_approved",
                    "Approve a Profile before completing SRA scope",
                    "sra",
                    f"/projects/{project_id}/sra",
                )
                _block(
                    blockers,
                    "risk_review_missing",
                    "Complete and review at least one SRA risk",
                    "sra",
                    f"/projects/{project_id}/risks",
                )
    if assessment is None and isinstance(declarations.get("sra"), dict):
        _block(
            blockers,
            "sra_not_approved",
            "Approve a Profile and complete SRA scope before close",
            "sra",
            f"/projects/{project_id}/sra",
        )
        _block(
...
def _result(
    ...
    close_decl: dict[str, Any],
    framework_id: str,
) -> dict[str, Any]:
    checks = []
    for name in validator_names:
        check = _CHECKS.get(name, "declaration")
        blocked = check == "declaration" or any(item.get("check") == check for item in blockers)
        checks.append({"name": name, "status": "Blocked" if blocked else "Ready", "check": check})
    links: list[dict[str, str]] = []
    seen: set[str] = set()
    for blocker in blockers:
        href = blocker.get("link")
        if not href or href in seen:
            continue
        seen.add(href)
        link = {"href": href, "label": blocker["detail"]}
        if "/records/" in href:
            link["record_id"] = href.split("/records/", 1)[1].split("/", 1)[0]
        elif href.endswith("/profile"):
            link["target"] = "profile"
        elif "/sra" in href or "/risks" in href:
            link["target"] = "sra"
        else:
    ...
    return {
        ...
        "informational": {name: "not_applicable" for name in close_decl.get("informational", [])},
    }
